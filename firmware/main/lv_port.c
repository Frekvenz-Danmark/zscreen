 ...
 */
/* zScreen: parameteren er tilfoejet.
 *
 * bsp_lcd_set_cb() forventer "bool (*)(void *)", men Seeeds eksempel
 * giver den en "bool (*)(void)". Det oversaetter med en advarsel og
 * virker i praksis paa Xtensa, fordi den ekstra parameter bare ligger i
 * et register funktionen ikke laeser. Men det ER udefineret opfoersel,
 * og det er ikke noget man vil have staaende i afbrydelsesstien paa en
 * skaerm der skal koere i aarevis. */
static bool lv_port_flush_ready(void *arg)
{
    (void)arg;
 ...
 * @return IRAM_ATTR
 */
/* zScreen: IRAM_ATTR er fjernet.
 *
 * Funktionen kaldes fra LVGL's egen opgave, ikke fra en afbrydelse, og
 * den laeser touch over I2C, som alligevel ikke ligger i IRAM. Den
...
static esp_err_t lv_port_tick_init(void)
{
    /* zScreen: const er fjernet.
     *
     * esp_timer's arg er en almindelig void*, saa en pegepind til
     * const-data giver en advarsel om at kvalifikatoren smides vaek.
     * Variablen er static og bliver aldrig skrevet til, saa der er
     * ingen grund til at caste os ud af det. */
